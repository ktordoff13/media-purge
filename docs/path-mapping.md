...
Media Review deletes files through **its own** filesystem, but your media server reports paths
...
| Media Review container | `/mnt/user/media` → `/media` |

Plex reports `Movie.mkv` as `/data/movies/Movie.mkv`. Media Review must translate that to
...
On approval, Media Review verifies the item's files are actually reachable before touching
...
`/mnt/user/media/.media-review-bin`). Moves are then instant renames. If it lives on another
